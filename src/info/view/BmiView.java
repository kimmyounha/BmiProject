...
public class BmiView extends JFrame {
	...
	class PanelAbove extends JPanel {
		public PanelAbove() {
			setBackground(new Color(120, 136, 151));
			...
			pans[0].add(lblWeight);
			pans[0].add(tfWeight);
			pans[0].add(lblkg);
			pans[1].add(lblHeight);
			pans[1].add(tfHeight);
			pans[1].add(lblcm);
	...
	class PanelBelow extends JPanel {
		public PanelBelow() {
			...
			ImageIcon icon = new ImageIcon("src/image/bmi_original.jpg");
...
//			BMIVO 객체에 입력 받은 체중과 키를 설정하고 bmi를 설정한다.
			...
			if (vo.getBmi() < 18.5) {
				bmiResult = "저";
				icon = new ImageIcon("src/image/under.jpg");
			} else if (vo.getBmi() < 24.9) {
				bmiResult = "정상";
				icon = new ImageIcon("src/image/normal.jpg");
			} else if (vo.getBmi() < 29.9) {
				bmiResult = "과";
				icon = new ImageIcon("src/image/over.jpg");
			} else if (vo.getBmi() < 34.9) {
				bmiResult = "비만";
				icon = new ImageIcon("src/image/obese.jpg");
			} else {
				bmiResult = "고도비만";
				icon = new ImageIcon("src/image/extremely.jpg");
			}

			lblResult.setText("<html>당신의 체중은 " + vo.getWeight() + "㎏, 키는 " + vo.getHeight() + "㎝이므로" + "<br>bmi지수는 "
					+ String.format("%.1f", vo.getBmi()) + "㎏/㎡이며, " + bmiResult + "체중입니다.</html>");
